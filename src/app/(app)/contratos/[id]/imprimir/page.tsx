...
export default async function ImprimirContratoPage({
  ...
  const status = contrato.status as Status;
  const totalUnidades = contrato.itens.reduce((s, i) => s + i.quantidade, 0);
  const valorTotal = contrato.itens.reduce(
      ...
      <div className="rounded-xl bg-white p-8 text-zinc-900 ring-1 ring-foreground/10 print:rounded-none print:p-0 print:ring-0">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <p className="font-mono text-sm text-zinc-600">{contrato.anoProv}</p>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-4">
          ...
          />
          <Info
            label="Tamanho do Painel"
            valor={contrato.tamanhoPainel || "—"}
          />
          ...
          <tbody>
            ...
            {contrato.itens.map((item, indice) => (
              <tr key={item.id} className="border-b border-zinc-200">
                ...
                <td className="py-2 pr-2 font-medium">{item.produto.item}</td>
                ...
                <td className="py-2 pr-2 text-right tabular-nums">
                  {brl(item.produto.valorRevenda)}
                </td>
                <td className="py-2 text-right tabular-nums">
                  {brl(item.quantidade * item.produto.valorRevenda)}
                </td>
              </tr>
            ))}
          </tbody>
